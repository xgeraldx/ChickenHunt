...
bool GameScene::init()
{
    ...
    _spawnTime = 0.0f;
    playerInfo = PlayerInfo::m_PlayerInfo->GetInstance();
    _score = 0;
    _currentAmmo = 2;
    ...
    _gameLogic->_scoreDisplay->setPosition(ccp(visibleSize.width * 0.9f, visibleSize.height * 0.94f));
...
void GameScene::spawnChicken()
{
    ...
    if ((rand() % 100) < 10) {
        for (int i=0; i<_foregroundChickenPool->count(); i++) {
            Chicken * tempChick = (Chicken *)_foregroundChickenPool->objectAtIndex(i);
            if(!tempChick->isVisible())
            {
                tempChick->setVisible(true);
                float x = rand() % (int) visibleSize.width * .8f;
                tempChick->_sprite->setPosition(ccp(x,tempChick->_sprite->boundingBox().size.height/2));
                tempChick->setVisible(true);
                tempChick->show();
                _foregroundChickens->addObject(tempChick);
                //this->addChild(tempChick->_sprite);
                break;
            }
...
void GameScene::createObjectPools()
{
    ...
    for (int i =0; i<9; i++) {
        FlyingChicken * tempChicken = new FlyingChicken(LARGE,false, LEFT);
        //FlyingChicken * tempChicken = new FlyingChicken();
        switch (rand() % 3) {
            case 1:
                ...
                this->addChild(tempChicken->_sprite,3);
                break;
            case 2:
            ...
            case 3:
                ...
                tempChicken->createAnimations();
                tempChicken->_sprite->setPosition(ccp(visibleSize.width/1.5,visibleSize.height/1.5));
                this->addChild(tempChicken->_sprite,2);
                break;
            default:
                tempChicken = new FlyingChicken(LARGE,false, LEFT);
                tempChicken->_sprite = CCSprite::createWithSpriteFrameName("chickFlyLarge0.png");
...
{
    unscheduleUpdate();
    CCDirector::sharedDirector()->pushScene(EndGameScene::scene());
}
void GameScene::ccTouchesBegan(CCSet* pTouches, CCEvent* event)
{
    ...
    }else
    {
        ...
        if (touch) {
            ...
            for(int i=0;i<_foregroundChickens->count();i++)
            {
                Chicken * fc = (Chicken *)_foregroundChickens->objectAtIndex(i);
                if (fc->_sprite->boundingBox().containsPoint(touch->getLocation())) {
                    fc->die();
                    _player->addScore(25);
                    _gameLogic->updateScore(_player->getScore());
                    _foregroundChickens->removeObjectAtIndex(i);
                    _player->addKill();
                    break;
                }else
                {
            ...
            playerInfo->setScore(_player->getScore());
